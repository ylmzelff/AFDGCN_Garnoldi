...
# A = torch.FloatTensor(A).to(args.device)  # Alternative way to create and move to device
...
hidden_dim = 64 
output_dim = 1
embed_dim = 16 #307 #34 #19#8   if you used adj, number of nodes should be entered here
cheb_k = 2
horizon = 1
num_layers = 1 
heads = 4
...
if torch.cuda.device_count() > 1:
    model = nn.DataParallel(model.cuda(), device_ids=[0, 1, 2, 3, 4])
