...
for basis in active_bases:
    ...
    if not df_all.empty:
        ...
        plt.figure(figsize=(12, 6))
        ax = sns.boxplot(x="Model", y="Validation MAE", data=df_all, palette="Set3")
        plt.title(f"Model Comparison – MAE Distribution ({basis_display})")
        plt.xlabel("Model")
        plt.ylabel("Validation MAE")
        plt.grid(True, linestyle="--", alpha=0.5)
        xticks = ax.get_xticks()
        for i, model in enumerate(df_all["Model"].unique()):
            ...
            ax.annotate(f"{mean_val:.2f}", (xticks[i], mean_val), ha='center', va='bottom', fontsize=10, fontweight='bold')
        out_path = f"figures/{basis.lower()}/all_models_comparison_{basis.lower()}_with_gfilters.png"
        os.makedirs(os.path.dirname(out_path), exist_ok=True)
        plt.tight_layout()
        plt.savefig(out_path, dpi=300)
        plt.close()
